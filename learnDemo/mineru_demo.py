...
def do_parse(
    ...
    if backend == "pipeline":
        ...
        # 通过pypdfium2库（一个高效的 PDF 处理工具）对二进制数据进行页面截取，避免解析无关页面，节省资源
        ...
        # 对预处理后的 PDF 进行全面分析，提取关键信息

        # 遍历推理结果，对每个PDF文件进行后处理和输出生成
        # 该循环处理每个PDF文件的推理结果，包括：
        # 1. 准备输出环境和工具
        # 2. 提取相关数据
        # 3. 转换为中间JSON格式
        # 4. 处理最终输出
...
def parse_doc(
    ...
    try:
        ...
        for path in path_list:
            ...
            # Path 是 Python 推荐的现代路径处理方式，让路径操作更简洁、跨平台、不易出错
            pdf_bytes = read_fn(path)
            # 把这两个文件的 “原始内容” 读成机器能处理的二进制数据
...
if __name__ == "__main__":
    ...
    pdf_files_dir = "./"
    output_dir = os.path.join(__dir__, "tryoutput")
    pdf_suffixes = ["pdf"]
    image_suffixes = ["png", "jpeg", "jp2", "webp", "gif", "bmp", "jpg"]

    doc_path_list = []
    for doc_path in Path(pdf_files_dir).glob("*"):
        # 这一行是保证代码的健壮性，利用guess_suffix_by_path函数判断文件后缀
